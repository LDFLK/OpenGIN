...
backup_postgres() {
    ...
    pg_dump_output=$(docker exec postgres pg_dump -U postgres -h localhost -d ${POSTGRES_DATABASE} --clean --if-exists -f "/var/lib/postgresql/backup/${backup_file}.sql" 2>&1)
    pg_dump_exit_code=$?
